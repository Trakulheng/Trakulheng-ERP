...
const APP_URL = process.env.NEXT_PUBLIC_APP_URL ?? "https://erp.trakulheng.com";
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.1)">
        ...
        <tr>
          <td style="padding:40px 40px 32px">
            ...
            <p style="margin:0 0 24px;color:#64748b;font-size:15px;line-height:1.6">
              Hi ${name ? name.split(" ")[0] : "there"}, we received a request to reset your password. Click the button below to set a new password. This link expires in <strong>1 hour</strong>.
            </p>
            <div style="text-align:center;margin:32px 0">
              <a href="${url}" style="display:inline-block;background:#7c3aed;color:#fff;text-decoration:none;padding:14px 36px;border-radius:10px;font-size:15px;font-weight:600">
                Reset password
              </a>
            </div>
        ...
        <tr>
          <td style="padding:24px 40px;text-align:center">
            <p style="margin:0;color:#94a3b8;font-size:12px">Or copy: <a href="${url}" style="color:#7c3aed">${url}</a></p>
          </td>
        </tr>
      </table>
    </td></tr>
  ...
  await resend.emails.send({
    from: FROM,
    to: email,
    subject: "Reset your Trakulheng password",
    html,
  });
}
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.1)">
        <tr>
          <td style="background:linear-gradient(135deg,#1d4ed8,#7c3aed);padding:32px 40px;text-align:center">
            <div style="display:inline-flex;align-items:center;gap:10px">
              <div style="width:40px;height:40px;background:rgba(255,255,255,.2);border-radius:10px;display:inline-block;line-height:40px;text-align:center;font-size:20px">📊</div>
              <span style="color:#fff;font-size:22px;font-weight:700;letter-spacing:-.5px">Trakulheng ERP</span>
            </div>
          </td>
        </tr>
        <tr>
          <td style="padding:40px 40px 32px">
            <h1 style="margin:0 0 8px;font-size:24px;font-weight:700;color:#0f172a">You're invited! 🎉</h1>
            <p style="margin:0 0 24px;color:#64748b;font-size:15px;line-height:1.6">
              Hi ${firstName}, an administrator has created an account for you on the <strong>Trakulheng ERP</strong> system. Click the button below to set up your password and get started.
            </p>
            <div style="text-align:center;margin:32px 0">
              <a href="${url}" style="display:inline-block;background:linear-gradient(135deg,#1d4ed8,#7c3aed);color:#fff;text-decoration:none;padding:16px 40px;border-radius:12px;font-size:16px;font-weight:600;letter-spacing:.2px;box-shadow:0 4px 14px rgba(124,58,237,.35)">
                Set Up Your Account →
              </a>
            </div>
            <div style="background:#f8fafc;border-radius:10px;padding:16px 20px;margin-bottom:16px">
              <p style="margin:0 0 4px;font-size:13px;font-weight:600;color:#475569">What you'll set up:</p>
              <ul style="margin:8px 0 0;padding-left:20px;color:#64748b;font-size:13px;line-height:2">
                <li>A secure password for your account</li>
                <li>A 6-digit PIN for quick access</li>
              </ul>
            </div>
            <p style="margin:0;color:#94a3b8;font-size:13px;line-height:1.6">
              This invitation link expires in <strong>7 days</strong>. If you weren't expecting this, please contact your system administrator.
            </p>
          </td>
        </tr>
        <tr><td style="padding:0 40px"><div style="height:1px;background:#f1f5f9"></div></td></tr>
  ...
  await resend.emails.send({
    from: FROM,
    to: email,
    subject: "You've been invited to Trakulheng ERP",
    html,
  });
}
