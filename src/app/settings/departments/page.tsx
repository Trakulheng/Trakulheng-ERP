...
function DeptModal({ initial, onClose, onSave, saving, error }: {
  initial?: Department; onClose: () => void;
  onSave: (d: Partial<Department>) => void; saving: boolean; error: string;
}) {
    ...
    <div className="fixed inset-0 h-screen z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="text-base font-semibold text-slate-900">{initial ? "Edit Department" : "Add Department"}</h2>
...
export default function DepartmentsPage() {
  ...
  const handleSave = async (data: Partial<Department>) => {
    ...
    try {
      ...
      const res = await fetch(isEdit ? `/api/settings/departments/${editing!.id}` : "/api/settings/departments", {
      ...
      setDepts((prev) => {
        ...
        return [...prev, json].sort((a, b) => a.name.localeCompare(b.name));
      });
      setModalOpen(false); setEditing(undefined);
      setToast(isEdit ? "Department updated." : "Department added.");
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      <Header title="Departments" subtitle="Manage departments used in employee records"
        actions={
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading && <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-slate-400">Loading…</td></tr>}
              {!loading && depts.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-12 text-center">
                  <Layers size={32} className="mx-auto text-slate-200 mb-3" />
                  <p className="text-sm text-slate-400">No departments yet. Add your first department to get started.</p>
                </td></tr>
              )}
              {depts.map((d) => (
                <tr key={d.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-500 text-xs">{d.description || "—"}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => { setSaveError(""); setEditing(d); setModalOpen(true); }} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700"><Pencil size={15} /></button>
                      <button onClick={() => setDeleteId(d.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-600"><Trash2 size={15} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {modalOpen && (
        <DeptModal initial={editing} onClose={() => { setModalOpen(false); setEditing(undefined); }}
      ...
      {deleteId && (
        <div className="fixed inset-0 h-screen z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <h3 className="text-base font-semibold text-slate-900 mb-1">Delete Department</h3>
            <p className="text-sm text-slate-500 mb-6">Remove <strong>{depts.find((d) => d.id === deleteId)?.name}</strong>? This won't affect existing employee records.</p>
