...
function CategoryModal({ initial, onClose, onSave, saving, error }: {
  initial?: ProductCategory; onClose: () => void;
...
export default function ProductCategoriesPage() {
  ...
  const [modal,     setModal]     = useState<ProductCategory | null | "new">(null);
  ...
  const fetchCats = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch("/api/settings/product-categories");
  ...
  const handleSave = async (data: { name: string; description: string }) => {
    ...
    try {
      ...
      const url = isEdit ? `/api/settings/product-categories/${(modal as ProductCategory).id}` : "/api/settings/product-categories";
  ...
  const handleDelete = async (id: string) => {
    try {
      await fetch(`/api/settings/product-categories/${id}`, { method: "DELETE" });
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">Add your first product category to get started.</p>
              <button onClick={() => { setSaveError(""); setModal("new"); }}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <p className="text-sm text-slate-500 mb-6">
              ...
              Existing products will keep their category label.
            </p>
